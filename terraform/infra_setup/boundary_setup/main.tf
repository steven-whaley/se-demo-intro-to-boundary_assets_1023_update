...
locals {
  ...
  boundary_instance_worker_config = <<-WORKER_CONFIG
    ...
    worker {
      ...
      recording_storage_path="/boundary_storage"
  ...
  cloudinit_config_boundary_instance_worker = {
    ...
    runcmd = [
      ...
      ["mkdir", "/boundary_storage"],
      ["chmod", "777", "/boundary_storage"]
    ]
  }
}
...
# Set up instance profile for Boundary worker that allows writing to S3 for session recording
resource "aws_iam_instance_profile" "session_recording_profile" {
  name = "session-recording-profile"
...
resource "aws_iam_policy" "session_recording_policy" {
  ...
  description = "Policy used by Boundary worker to write to S3 bucket for session recording"
  policy      = data.aws_iam_policy_document.session_recording_policy.json
}

resource "aws_iam_role" "session_recording_role" {

  name = "session_recording_role"
  path = "/"
...
resource "aws_iam_policy_attachment" "session_recording_policy" {
  name       = "session-recording-policy-attachment"
  roles      = [aws_iam_role.session_recording_role.name]
